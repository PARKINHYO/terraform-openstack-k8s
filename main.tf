...
################################################################################
# network, flavor
# resource name: tf_k8s, tf_k8s_flavor
################################################################################
...
resource "openstack_compute_flavor_v2" "tf_k8s_flavor_control_plane" {
  name      = "k8s-type-control-plane"
  ram       = "8192"
  vcpus     = "8"
  disk      = "20"
  is_public = true
...
resource "openstack_compute_flavor_v2" "tf_k8s_flavor_worker_node" {
  name      = "k8s-type-worker-node"
  ram       = "4096"
  vcpus     = "4"
  disk      = "20"
  is_public = true
...
resource "openstack_compute_instance_v2" "worker_node" {
  ...
  image_name      = var.image
  flavor_name     = var.flavor_worker_node
  key_pair        = openstack_compute_keypair_v2.tf_k8s.name
  security_groups = ["${openstack_networking_secgroup_v2.tf_k8s.name}"]
